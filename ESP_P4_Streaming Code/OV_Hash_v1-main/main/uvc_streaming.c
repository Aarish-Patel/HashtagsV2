...
static esp_err_t on_stream_start(uvc_format_t uvc_format, int width, int height, int rate, void *cb_ctx)
{
    ...
    /* Tell HTTP server to yield camera/encoder if self-capturing */
    http_server_notify_uvc_start();
...
static void on_stream_stop(void *cb_ctx)
{
    ...
    /* Tell HTTP server it can resume self-capture */
    http_server_notify_uvc_stop();
}
